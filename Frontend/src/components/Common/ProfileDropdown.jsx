...
function ProfileDropdown() {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div 
          className="absolute right-0 mt-2 w-96 bg-custom-white rounded-xl shadow-xl border border-custom-light-gray z-50 overflow-hidden"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-custom-charcoal to-custom-dark-gray p-4 text-custom-white">
            <div className="flex items-center gap-3">
              {clerkUser?.profileImageUrl ? (
                ...
                <div className="w-12 h-12 rounded-full bg-custom-light-gray flex items-center justify-center text-custom-charcoal text-lg font-bold">
                  {clerkUser?.firstName?.charAt(0).toUpperCase() || 'U'}
                </div>
              )}
              <div>
                <p className="font-medium">{clerkUser?.fullName || 'User'}</p>
                <p className="text-sm text-custom-light-gray">{clerkUser?.primaryEmailAddress?.emailAddress}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 max-h-96 overflow-y-auto">
            {isLoading && activeTab === 'profile' && <p className="text-center text-custom-dark-gray py-4">Loading...</p>}
            ...
            {!isLoading && activeTab === 'profile' && userProfile && (
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-custom-charcoal">{userProfile.name}</p>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-custom-dark-gray mb-1">Email</label>
                  <p className="text-sm text-custom-charcoal">{userProfile.email}</p>
                </div>
                <div>
              ...
              <div className="space-y-4">
                {!isEditing && userPreferences ? (
                  <div className="space-y-3">
                    <div className="bg-custom-off-white p-3 rounded-lg">
                      <label className="block text-xs font-semibold text-custom-dark-gray mb-1">Skin Type</label>
                      <p className="text-sm text-custom-charcoal font-medium">{getFormattedValue(userPreferences.skinType)}</p>
                    </div>
                    ...
                    <div className="bg-custom-off-white p-3 rounded-lg">
                      ...
                      <p className="text-sm text-custom-charcoal font-medium">{getFormattedValue(userPreferences.knownAllergies)}</p>
                    </div>

                    <div className="bg-custom-off-white p-3 rounded-lg">
                      <label className="block text-xs font-semibold text-custom-dark-gray mb-1">Product Change Rate</label>
                      <p className="text-sm text-custom-charcoal font-medium">{getFormattedValue(userPreferences.productChangeRate)}</p>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full mt-4 px-4 py-2 bg-custom-charcoal text-custom-white rounded-lg hover:bg-custom-black transition font-medium text-sm"
                    >
                      Edit Skin Details
                    </button>
                  </div>
                  ...
                  <form className="space-y-3">
                    <div>
                      <label className="block text-xs font-semibold text-custom-charcoal mb-2">Skin Type</label>
                      <select
                    ...
                    <div>
                      <label className="flex items-center gap-2 text-xs font-semibold text-custom-charcoal cursor-pointer">
                        <input
                          ...
                          className="w-4 h-4 rounded border-custom-light-gray"
                        />
                        High Sensitivity
                      </label>
                    </div>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-custom-charcoal mb-2">Known Allergies</label>
                        ...
                        {allergyOptions.map(allergy => (
                          <label key={allergy} className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              className="w-4 h-4 rounded border-custom-light-gray"
                            />
                            <span className="text-sm text-custom-dark-gray">{allergy}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 mt-4">
                      <button
                        ...
                        className="flex-1 px-4 py-2 border border-custom-light-gray text-custom-charcoal rounded-lg hover:bg-custom-off-white transition font-medium text-sm"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        disabled={isLoading}
                        className="flex-1 px-4 py-2 bg-custom-charcoal text-custom-white rounded-lg hover:bg-custom-black transition font-medium text-sm disabled:opacity-50"
                      >
          ...
          {/* Footer */}
          <div className="border-t border-custom-light-gray p-4 bg-custom-off-white">
            <button
              type="button"
              onClick={handleSignOut}
              className="w-full px-4 py-2 text-custom-charcoal border border-custom-light-gray rounded-lg hover:bg-custom-white transition font-medium text-sm"
            >
              Sign Out
            </button>
          </div>
